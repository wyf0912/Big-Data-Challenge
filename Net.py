...
class LSTMTagger(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim):
        ...
        self.liner_1 = nn.Linear(self.num_layers * hidden_dim, 2 * hidden_dim)
    ...
    def forward(self, batch_data):
        ...
        for data in batch_data:
            ...
            #if end_time - start_time > 14:  # 防止数据过长
            #    start_time = end_time - 14
            ...
            self.hidden = (torch.cat((reg_type.unsqueeze(0).unsqueeze(0), Variable(torch.zeros(self.num_layers - 1, 1, 32)).cuda())),
                           Variable(torch.zeros(self.num_layers, 1, self.hidden_dim)).cuda())
            ...
            x = F.relu(self.liner_1(torch.cat([x[0] for x in self.hidden[0]])))
...
def train():
    ...
    optimizer = optim.SGD(lstm.parameters(), lr=0.3)
...
def make_predict():
    ...
    lstm.load_state_dict(torch.load('./saved_model/lstm_layer4+2(input12) 0.814437.pkl'))
    f = open('result.csv', 'w')
